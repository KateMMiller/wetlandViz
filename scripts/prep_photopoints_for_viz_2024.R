...
sitedata <- read.csv('./wetlandViz/data/Sentinel_and_USA-RAM_Sites_2023.csv')
sitedata_old <- sitedata %>% filter(Panel != 3)

all_photos <- data.frame('file' = list.files("./wetlandViz/www/", pattern='JPG$'))

sitedata_2024 <- sitedata %>% filter(Panel == 3) %>% select(Site:Cowardin_Class)
panel3 <- paste(sitedata_2024$Label, sep = "", collapse = "|")

photos_2024 <- all_photos |> filter(grepl("_2024", file))

photos_2024 <- photos_2024 %>% mutate(file2 = str_remove(file, ".JPG"),
...
photos_2024
#++++ ENDED HERE ++++ Just need to make columns the same between old and 2024 then bind.

sitedata_update <-left_join(sitedata_2023, photos_2024, by = "Label", all.x = TRUE, all.y = TRUE)
