...
# welld and well_stats files are Generated by "
...
welld <- read.csv('./data/well_prec_data_2013-2024.csv')
well_stats <- read.csv('./data/well_growing_season_stats_2013-2024_long.csv')
...
sitedata <- read.csv('./data/Sentinel_and_USA-RAM_Sites_2024.csv')
tail(sitedata)
...
# Pull in veg data
vmmi <- read.csv('./data/vmmi_2020-2024.csv') #includes 2019 data
sppdata <- read.csv("./data/Sentinel_and_RAM_species_data_2020-2024.csv") #sensitive spp. removed
sampleyears <- c(2020:2024) # most recent survey of available data
...
vmmimap <-
  ...
  select(Label, Site_Type, Longitude, Latitude, Year,
         Mean_C:VMMI_Rating, North_View:West_View)

tail(vmmimap)

sppmap1 <- merge(sppdata, sitedata[,c('Label', 'HGM_Class', 'HGM_Subclass', 'Cowardin_Class')],
                 by = 'Label', all.x = T, all.y = T)
...
sppinv <- sppmap %>%  
  mutate(inv_num = ifelse(PctFreq > 0 & Invasive == TRUE, 1, 0)) %>% 
  select(Label:Site_Type, inv_num) %>% 
  group_by(Label, Longitude, Latitude, Site_Type) %>% 
  summarise(num_inv = sum(inv_num), 
            inv_present = ifelse(num_inv > 0,'Present','Absent'), .groups = 'drop')
