...
sentsites<-data.frame(sitename = c("Big Heath", "Duck Pond", "Gilmore Meadow", "Heath Brook", 
                                   "Hodgdon Swamp","Little Hunter's Brook", "New Mills Meadow", 
                                   "Western Mtn. Swamp"), 
...
sitemap <- merge(sitedata, vmmi[ ,c("Label", "Year")], by = 'Label', all.x = TRUE) %>% 
  filter(Year %in% sampleyears) %>% droplevels()

# Prep veg data for Map panel
vmmimap <- merge(sitedata, vmmi, by = c('Label'), all.x = TRUE, all.y = TRUE) %>% 
  mutate_at(vars(Mean_C:VMMI), list(~round(., 1))) %>% 
  mutate(VMMI_Rating = factor(VMMI_Rating, levels = c('Good','Fair','Poor'))) %>% 
  arrange(Site_Type, Label) %>% filter(Year %in% sampleyears) %>% 
  select(Label, Site_Type, Longitude, Latitude, Year, Mean_C:VMMI_Rating, North_View:West_View)

sppmap1 <- merge(sppdata, sitedata[,c('Label', 'HGM_Class', 'HGM_Subclass', 'Cowardin_Class')], 
                 by = 'Label', all.x = TRUE, all.y = TRUE)
...
sppinv <- sppmap %>%  mutate(inv_num = ifelse(PctFreq > 0 & Invasive == TRUE, 1, 0)) %>% 
                      select(Label:Site_Type, inv_num) %>% 
                      group_by(Label, Longitude, Latitude, Site_Type) %>% 
